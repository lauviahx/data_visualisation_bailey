...
async function chartItA() {
    ...
    for (let i = 0; i < data.length; i++) { // loop through until reached end of the database
        if (data[i].line == 'A') { // find and use only data for line A
...
async function chartItB() { // function to create chart for line B, same principles as other
    const response = await fetch('/api');
    const data = await response.json();
...
async function chartItC() { // function to create chart for line C, same principles as other
    const response = await fetch('/api');
    const data = await response.json();
...
async function chartItD() { // function to create chart for line D, same principles as other
    const response = await fetch('/api');
    const data = await response.json();
